			...
			mess = {
				...
				only: {
					...
					Badmin: 'Este comando sÛ pode ser usado quando o bot se torna adm!',
                                        daftarB: `Ol·! VocÍ n„o est· registrado no banco de dados. Comando : ${prefix}daftar nome|idade Exemplo : ${prefix}daftar Ryz|17`,
				}
			}
               ...
               if (data.error) return reply(data.error)
				...
				case 'att':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `[ Propriet·rio do bot disse ] ${body.slice(4)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `[ Propriet·rio do bot disse ] ${body.slice(4)}`)
						}
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of groupMembers) {
							client.sendMessage(_.jid, buff, image, {caption: `[ Propriet·rio do bot disse ] Grupo : ${groupName}\n\n${body.slice(6)}`})
						}
						reply('')
					} else {
						for (let _ of groupMembers) {
							sendMess(_.jid, `[ Propriet·rio do bot disse ] Grupo : ${groupName}\n\n${body.slice(6)}`)
						}
						reply('Mensagem enviada com sucesso para todos os chats')
					}
					break

		...
		try {
            ...
            try {
                ...
                if (requiredXp <= getLevelingXp(sender)) {
                    ...
                    await reply(`„Äå SUBIU DE NÕVEL „Äç\n\n‚û∏ Nome: ${sender}\n‚û∏ XP: ${getLevelingXp(sender)}\n‚û∏ NÌvel: ${getLevel} -> ${getLevelingLevel(sender)}\n\nParabÈns!! üéâüéâ`)
                }
            } catch (err) {
                console.error(err)
            }
               ...
               if (data.error) return reply(data.error)
				...
				case 'att':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `[ Propriet·rio do bot disse ] „Äç\n\n${body.slice(4)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `[ Propriet·rio do bot disse ] „Äç\n\n${body.slice(4)}`})
						}
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of groupMembers) {
							client.sendMessage(_.jid, buff, image, {caption: `„Äå [ Propriet·rio do bot disse ] „Äç\nGroup : ${groupName}\n\n${body.slice(6)}`})
						}
						reply('')
					} else {
						for (let _ of groupMembers) {
							sendMess(_.jid, `„Äå [ Propriet·rio do bot disse ] „Äç\nGroup : ${groupName}\n\n${body.slice(6)}`)
						}
						reply('Mensagem enviada com sucesso para todos os chats')
					}
					break

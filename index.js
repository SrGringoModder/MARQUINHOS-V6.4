		...
		try {
			...
			mess = {
				...
				only: {
					...
					Badmin: 'Este comando s� pode ser usado quando o bot se torna adm!',
            ...
            try {
                ...
                if (requiredXp <= getLevelingXp(sender)) {
                    ...
                    await reply(`「 SUBIU DE N�VEL 」\n\n➸ Nome: ${sender}\n➸ XP: ${getLevelingXp(sender)}\n➸ N�vel: ${getLevel} -> ${getLevelingLevel(sender)}\n\nParab�ns!! 🎉🎉`)
                }
            } catch (err) {
                console.error(err)
            }
               ...
               break
				case 'rebaixar':
                  ...
                  case 'promover':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Pedido recebido, adicionando posi��o como adm :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`Pedido recebido, adicionando posi��o como adm : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break
				  case 'wa.me':
				...
				if (args.length < 1) return reply('onde est� o link do YouTube?')
					...
					buffer = await getBuffer(anu.thumb)
				 	anu = await fetchJson(`https://tobz-api.herokuapp.com/api/ytv?url=${tels}&apikey=BotWeA`, {method: 'get'})
                                        if (!isUser) return reply(mess.only.daftarB)
					hasil = `T�tulo : ${anu.title}\nTamanho do arquivo : ${anu.filesize}\nResolu��o : ${anu.resolution}\nTipo : ${anu.ext}\nLink : ${anu.result}`					
	      ...
	      tels = body.slice(6)
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'chord':
					if (args.length < 1) return reply('cad� o t�tulo da m�sica mano?')
                                        if (!isUser) return reply(mess.only.daftarB)
					tels = body.slice(7)					
					...
					break
				case 'lirik':
					if (args.length < 1) return reply('Cad� a letra da m�sica, mano?')
                                        if (!isUser) return reply(mess.only.daftarB)
					tels = body.slice(7)
			...
			case 'igstalk':
               ...
               client.updatePresence(from, Presence.composing) 
              options = {
          text: `O propriet�rio do grupo � : @${from.split("-")[0]}`,
          contextInfo: { mentionedJid: [from] }
           ...
           break
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('A que voc� deseja adicionar um g�nio??')
			...
			case 'fechargrp':
               ...
               if (anu.error) return reply(anu.error)
				...
				case 'logowolf2':
					...
					break
                case 'truth':
					const trut =['J� gostou de algu�m? quanto tempo? ',' Se poss�vel ou se voc� quiser, em gc/fora da gc, de quem voc� ser� amigo??(pode ser tipo / sma diferente)','Qual � o seu maior medo? ',' Voc� j� gostou de algu�m e sentiu que essa pessoa tamb�m gosta de voc�? ',' Qual � o nome do ex-namorado do seu amigo que voc� gostava secretamente? ',' Voc� nunca roubou seu dinheiro ou?']
			       ...
			       reply(anu.result.jadwalTV)
					break
                                ...
                                case 'randomkpop':
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Esta � a irm� kpop...'})
                                        break
                                case 'fml2':

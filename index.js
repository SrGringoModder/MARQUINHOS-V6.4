...
${mdata.subject}`
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: 'Em processo, agurade',
				success: 'Funciona',
                                levelon: 'Habilitar nivelamento',
				leveloff: 'Desabilitar nivelamento',
				levelnoton: 'Nivelamento n„o ativo',
				levelnol: 'NÕVEL DE IRM√O AINDA',
				error: {
					stick: ' Falha, ocorreu um erro ao converter a imagem para sticker',
					Iv: 'Link inv·lido'
				},
				only: {
					group: 'Este comando sÛ pode ser usado em grupos!',
					ownerG: 'Este comando sÛ pode ser usado pelo grupo propriet·rio!',
					ownerB: 'Este comando sÛ pode ser usado pelo propriet·rio do bot!',
					admin: 'Este comando sÛ pode ser usado por administradores de grupo!',
					Badmin: 'Este comando sÛ pode ser usado quando o bot se torna adm!',
                                        daftarB: `DAFTAR \nOl· !\nVocÍ n„o est· registrado no banco de dados, \n\nComando : ${prefix}daftar nama|umur\nExemplo : ${prefix}daftar Ryz|17`,
				}
			}
            ...
            try {
                ...
                if (requiredXp <= getLevelingXp(sender)) {
                    ...
                    await reply(`LEVEL UP\n\nNome: ${sender}\nXP: ${getLevelingXp(sender)}\nNÌvel: ${getLevel} -> ${getLevelingLevel(sender)}\n\nParabÈns!!`)
                }
            } catch (err) {
                console.error(err)
            }
               ...
               break
				case 'demote':
                  ...
                  case 'promote':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Pedido recebido, adicionando posiÁ„o como adm :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`Pedido recebido, adicionando posiÁ„o como adm : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break
				  case 'wa.me':
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'owner':
               ...
               client.sendMessage(from, 'Aqui est· o n˙mero do meu dono mano, salve mano, salve mais tarde,MessageType.text, { quoted: mek} )
                break
	case 'hidetag':
                client.updatePresence(from, Presence.composing) 
                if (!isUser) return reply(mess.only.daftarB)
              ...
              await client.sendMessage(from, options, text)
               break
					...
					try {
						...
						reply('nome de usu·rio tidak v·lido ')
					}
					break
				case 'snowwrite':
		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
				...
				case 'ytmp3':
					...
					break
					case 'ytmp4':
				if (args.length < 1) return reply('onde est· o link do YouTube?')
					tels = body.slice(7)				
					reply(mess.wait)
					buffer = await getBuffer(anu.thumb)
				 	anu = await fetchJson(`https://tobz-api.herokuapp.com/api/ytv?url=${tels}&apikey=BotWeA`, {method: 'get'})
                                        if (!isUser) return reply(mess.only.daftarB)
					hasil = `TÌtulo : ${anu.title}\nTamanho do arquivo : ${anu.filesize}\nResoluÁ„o : ${anu.resolution}\nTipo : ${anu.ext}\nLink : ${anu.result}`					
				...
				case 'nulis': 
				case 'tulis':
					if (args.length < 1) return reply('Eu te disse para escrever o que irm„o?')
                                        if (!isUser) return reply(mess.only.daftarB)
					teks = body.slice(7)
					reply(mess.wait)
	...
	case 'kbbi':
	      ...
	      tels = body.slice(6)
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'info':
					...
					break
				case 'blocklist':
					teks = 'Lista de n˙meros bloqueados :\n'
                   ...
                   case 'chatlist':
					client.updatePresence(from, Presence.composing)  
					teks = 'Llista de n˙meros de bate-papo :\n'
				...
				case 'tts':
				   ...
				   if (args.length < 1) return client.sendMessage(from, 'Qual È o cÛdigo da linguagem, tio?', text, {quoted: mek})
                                   if (!isUser) return reply(mess.only.daftarB)
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return client.sendMessage(from, 'Onde est· o texto om', text, {quoted: mek})
					dtt = body.slice(8)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
					? reply('A maior parte do texto È tio')
				...
				client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, ' Mencione tudo „Äô‚ú™‚ïê‚ïê\n‚ï†‚û•'+teks+'‚ïö‚ïê„Äò - - - - „Äô', text, {quoted: mek})
					break
                case 'tagall3':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
					...
					client.sendMessage(from, 'Mencione tudo „Äô‚ú™‚ïê‚ïê\n‚ï†‚û•'+teks+'‚ïö‚ïê„Äò - - - - - „Äô', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				client.updatePresence(from, Presence.composing) 
					...
					reply(' Mencione tudo „Äô‚ú™‚ïê‚ïê\n‚ï†‚û•'+teks+'‚ïö‚ïê„Äò - - - - - „Äô')
					break
				case 'send':
					var pc = body.slice(6)
				...
				case 'att':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `[ Propriet·rio do bot disse ]\n\n${body.slice(4)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `[ Propriet·rio do bot disse ]\n\n${body.slice(4)}`)
						}
				...
				case 'chord':
					if (args.length < 1) return reply('cadÍ o tÌtulo da m˙sica mano?)
                                        if (!isUser) return reply(mess.only.daftarB)
					tels = body.slice(7)					
					...
					break
				case 'lirik':
					if (args.length < 1) return reply('CadÍ a letra da m˙sica, mano?')
                                        if (!isUser) return reply(mess.only.daftarB)
					tels = body.slice(7)
			...
			case 'igstalk':
               ...
               client.updatePresence(from, Presence.composing) 
              options = {
          text: `O propriet·rio do grupo È : @${from.split("-")[0]}`,
          contextInfo: { mentionedJid: [from] }
           ...
           break
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('A que vocÍ deseja adicionar um gÍnio??')
					if (args[0].startsWith('08')) return reply('Use o cÛdigo do paÌs')
					try {
						num = `${args[0].replace(/ /g, '')}@s.whatsapp.net`
						client.groupAdd(from, [num])
					} catch (e) {
						console.log('Error :', e)
						reply('Falha ao adicionar destino, talvez porque È privado')
					}
					break
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						client.groupRemove(from, mentioned)
					client.sendMessage(mentioned, 'yahaha Lu kekickòÇ', text)
					}
					break
				case 'exe':
	              client.updatePresence(from, Presence.composing) 
				...
				case 'ocr':
                    ...
                    reply('Problemas foram relatados ao propriet·rio do BOT, relatÛrios falsos n„o ser„o respondidos.')
                    break
               case 'apakah':
               client.updatePresence(from, Presence.composing) 
	    ...
	    case 'kapankah':
			...
			case 'fechargrp':
				...
				case 'toimg':
                	...
                	client.updatePresence(from, Presence.composing) 
                        if (!isUser) return reply(mess.only.daftarB)
					if (!isQuotedVideo) return reply('responder vÌdeo hum')
					reply(mess.wait)
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await client.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp4')
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply(' Falha ao converter vÌdeo para mp3')
						buffer = fs.readFileSync(ran)
                ...
                case 'ninjalogo':
                      if (args.length < 1) return reply('CadÍ o texto?')
                      if (!isUser) return reply(mess.only.daftarB)
                      gh = body.slice(11)
                      gl1 = gh.split("|")[0];
                      gl2 = gh.split("|")[1];
               ...
               if (!isUser) return reply(mess.only.daftarB)
               if (anu.error) return reply(anu.error)
                 infomp3 = `Musica encontrada !!!\nTÌtulo : ${anu.result.title}\nDuraÁ„o : ${anu.result.duration}\nTamanho : ${anu.result.size}\n\nAguarde!!!`
                ...
                break
                   ...
                   break
					...
					break
                                  case 'registrar':
					client.updatePresence(from, Presence.composing)
					if (isUser) return reply('VocÍ j· est· registrado')
					if (args.length < 1) return reply(`Par‚metros errados\nComando : ${prefix}daftar nama|umur\nExemplo : ${prefix}daftar Caliph|12`)
					var reg = body.slice(8)
					var jeneng = reg.split("|")[0];
					var umure = reg.split("|")[1];
						user.push(sender)
						fs.writeFileSync('./database/json/user.json', JSON.stringify(user))
						client.sendMessage(from, `\`\`\`Cadastrado com sucesso com SN: TM08GK8PPHBSJDH10J\`\`\`\n\n\`\`\`Em ${date} ${time}\`\`\`\n\`\`\`[Nome]: ${jeneng}\`\`\`\n\`\`\`[N˙mero]: wa.me/${sender.split("@")[0]}\`\`\`\n\`\`\`[Era]: ${umure}\`\`\`\n\`\`\`Para usar um bot\`\`\`\n\`\`\`Por favor\`\`\`\n\`\`\`enviar ${prefix}help\`\`\`\n\`\`\`\nTotal de usu·rios ${user.length}\`\`\``, text, {quoted: mek})
					break
                                case 'bemvindo':
					if (!isGroup) return reply(mess.only.group)
                                        if (!isUser) return reply(mess.only.daftarB)
					...
					fakta = body.slice(1)
                                        if (!isUser) return reply(mess.only.daftarB)
					const fakta =['A massa da Terra atinge 5.972.190.000.000.000.000.000.000 kg. Mesmo sendo t„o pesado, o fato È que a Terra tem uma velocidade de 107.281 km por hora para circundar o sol. Muito r·pido, certo? ',' A grande massa da Terra È dominada pela poeira espacial e a bateria pode ser reduzida]
					...
					if (args.length < 1) return reply(mess.blank)
                                        if (!isUser) return reply(mess.only.daftarB)
					tels = body.slice(7)
					if (tels.length > 15) return reply('O texto È muito longo, atÈ 20 caracteres')
					reply(mess.wait)
				...
				case 'firetext':
					if (args.length < 1) return reply(mess.blank)
                                        if (!isUser) return reply(mess.only.daftarB)
					tels = body.slice(7)
					if (tels.ength > 10) return reply('O texto È longo, atÈ 9 caracteres')
					reply(mess.wait)
				...
				case 'logowolf':
					...
					if (args.length < 1) return reply(`teksnya mana? Exemplo ${prefix}logowolf Nazwa|Canss`)
					reply(mess.wait)
				...
				case 'logowolf2':
					...
					var gbl1 = gh.split("|")[0];
					var gbl2 = gh.split("|")[1];
					if (args.length < 1) return reply('CadÍ o texto, hum\nExemplo: ${prefix}phlogo |Nazwa|Canss')
					reply(mess.wait)
					...
					break
                case 'truth':
					const trut =['J· gostou de alguÈm? quanto tempo? ',' Se possÌvel ou se vocÍ quiser, em gc/fora da gc, de quem vocÍ ser· amigo??(pode ser tipo / sma diferente)','Qual È o seu maior medo? ',' VocÍ j· gostou de alguÈm e sentiu que essa pessoa tambÈm gosta de vocÍ? ',' Qual È o nome do ex-namorado do seu amigo que vocÍ gostava secretamente? ',' VocÍ nunca roubou seu dinheiro ou?']
					const ttrth = trut[Math.floor(Math.random() * trut.length)]
					truteh = await getBuffer(`https://i.ibb.co/305yt26/bf84f20635dedd5dde31e7e5b6983ae9.jpg`)
					client.sendMessage(from, truteh, image, { caption: '*Truth*\n\n'+ ttrth, quoted: mek })
					break
                                case 'dare':
					const dare =['Envie uma mensagem para o seu ex e diga "Eu ainda gosto de vocÍ ',' ligue para Crush / namorada agora e ss para o jogador ',' papai para um membro do grupo ',' Diga 'VOC  EST¡ BONITO E N√O FOI AMIG¡VEL' ss ']
            ...
            case 'leveling':
                ...
                if (args[0] === '1') {
                    if (isLevelingOn) return reply('o recurso de nÌvel j· estava ativo antes')
            ...
            break
                                ...
                                case 'nsfwtrap':
                                        try{
                                                ...
                                                buffer = await getBuffer(res.result)
                                                client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Aqui est· a foto, mano...'})
                                        } catch (e) {
                                                console.log(`Error :`, color(e,'red'))
                                                reply('ERROR')
                                        }
                                ...
                                case 'lionlogo':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        var teks2 = gh.split("|")[1];
                                        if (args.length < 1) return reply('onde est· o texto hum\nExemplo: ${prefix}lionlogo Nazwa|Canss')
                                        reply(mess.wait)
                                ...
                                case 'jokerlogo':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('onde est· o texto hum\nExemplo: ${prefix}jokerlogo NazwaCanss')
                                        reply(mess.wait)
			       ...
			       reply(anu.result.jadwalTV)
					break
                                case 'afk':
                                        ...
                                        const tag = {
                                                text: `@${nom.split("@s.whatsapp.net")[0]} MEDIUM AFK ${tels} N√O PERTURBE SIM`,
                                ...
                                case 'burnpaper':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('onde est· o texto hum\nExemplo: ${prefix}burnpaper NazwaCanss')
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'lovepaper':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('onde est· o texto hum\nExemplo: ${prefix}lovepaper NazwaCanss')
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'woodblock':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('onde est· o texto hum\nExemplo: ${prefix}woodblock NazwaCanss')
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'qowheart':
                                        var gh = body.slice(9)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('onde est· o texto hum\nExemplo: ${prefix}qowheart NazwaCanss')
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'mutgrass':
                                        var gh = body.slice(9)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('onde est· o texto hum\nExemplo: ${prefix}mutgrass NazwaCanss')
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'undergocean':
                                        var gh = body.slice(12)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('onde est· o texto hum\nExemplo: ${prefix}undergocean NazwaCanss')
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'metalictglow':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('onde est· o texto hum\nExemplo: ${prefix}metalictglow NazwaCanss')
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'randomkpop':
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Esta È a irm„ kpop...'})
                                        break
                                case 'fml2':
					...
					break
				case 'ttp':
					if (args.length < 1) return reply('O texto est· onde um?')
					ranp = getRandom('.png')
					rano = getRandom('.webp')
				...
				case 'clonar':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('Marque o alvo que vocÍ deseja clonar ')
				...
				default:
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[ADVERTIR]','red'), 'Comando n„o registrado de', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
			console.log('Error : %s', color(e, 'red'))

    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'owner':
               ...
               client.sendMessage(from, 'Aqui est· o n˙mero do meu dono mana, tire suas duvidas com ele... ',MessageType.text, { quoted: mek} )
                break
	case 'hidetag':
                client.updatePresence(from, Presence.composing) 
                if (!isUser) return reply(mess.only.daftarB)
	      ...
	      tels = body.slice(6)
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'chord':
					if (args.length < 1) return reply('cadÍ o tÌtulo da m˙sica mana?)
                                        if (!isUser) return reply(mess.only.daftarB)
					tels = body.slice(7)					
					...
					break
				case 'lirik':
					if (args.length < 1) return reply('CadÍ a letra da m˙sica, mana?')
                                        if (!isUser) return reply(mess.only.daftarB)
					tels = body.slice(7)

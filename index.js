			...
			mess = {
				...
				only: {
					...
					Badmin: 'Este comando so pode ser usado quando o bot se torna adm!',
                                      daftarB: `Ola !\nVoce nao esta cadastrado no banco de dados, \n\nComando : ${prefix}cadastrar nome|idade\nExemplo : ${prefix}cadastrar Joao|17`,
				}
			}
                ...
                break
                   ...
                   break
					...
					break
                                  case 'cadastrar':
					client.updatePresence(from, Presence.composing)
					if (isUser) return reply('Voce ja esta cadastrado')
					...
					var umure = reg.split("|")[1];
						...
						client.sendMessage(from, Cadastrado com sucesso: \n\nEm ${date} ${time}\n[Nome]: ${jeneng}\n[Nmero]: wa.me/${sender.split("@")[0]}\n[Era]: ${umure}\nPara usar um bot\nPor favor\nenviar ${prefix}help\n\nTotal de usuarios ${user.length}`, text, {quoted: mek})
					break
                                case 'bemvindo':
					if (!isGroup) return reply(mess.only.group)
                                        if (!isUser) return reply(mess.only.daftarB)
